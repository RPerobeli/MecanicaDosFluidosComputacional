    ...
    for j= 1:numNos
        if(j==1)
            for i = 2:numNos-1
                ...
                f(i-1) = (1-2*sigma)*T(i,j)+sigma*(rhox(i,j+1)+1)*T(i,j+1) + sigma*(1-rhox(i,j+1))*T(i,j+1);
            end
        else if(j == numNos)
                for i = 2:numNos-1
                    %notar que o rhox e rhoy variam de 2 até numNos-1            
                ...
                for i = 2:numNos-1
                    ...
                    f(i-1) = (1-2*sigma)*T(i,j)+sigma*(rhox(i,j)+1)*T(i,j+1) + sigma*(1-rhox(i,j))*T(i,j-1);
                end
            end
    ...
    for i= 2:numNos-1
        for j = 1:numNos
            if(j ==1)
                ...
                p(1,j) = 1+2*sigma; %diagonal principal
                q(1,j) = -sigma*2; %diagonal superior
                r(j) = (1-2*sigma)*T_meio(i,j)+sigma*(1-rhoy(i,j+1))*T_meio(i+1,j) + sigma*(1+rhoy(i,j+1))*T_meio(i-1,j);
            else if (j == numNos)
                 %notar que o rhox e rhoy variam de 2 até numNos-1
...
%             c = sigma*(rhoy(2:numNos-1,j)-Pr);%diagonal superior 
        ...
        for i = 2:numNos-1
            ...
            b(1,i-1) = 1+2*Pr*sigma; %diagonal principal
            c(1,i-1) = sigma*(rhoy(i,j)-Pr); %diagonal superior
            
            f(i-1) = (1-2*Pr*sigma)*w(i,j)+sigma*(rhox(i,j)+Pr)*w(i,j+1) + sigma*(Pr-rhox(i,j))*w(i,j-1)+ Ra*Pr*theta_x(i,j);
        end
        %COLOCAR AQUI OS TERMOS FONTE QDO i=1 E i=4
    ...
    for i= 2:numNos-1
        for j = 2:numNos-1
            ...
            b(1,j-1) = 1+2*Pr*sigma; %diagonal principal
            c(1,j-1) = -sigma*(rhox(i,j)+Pr); %diagonal superior
            
            f(j-1) = (1-2*Pr*sigma)*w_meio(i,j)+sigma*(Pr-rhoy(i,j))*w_meio(i+1,j) + sigma*(Pr+rhoy(i,j))*w_meio(i-1,j) + Ra*Pr*theta_x(i,j);
        end
        %COLOCAR AQUI OS TERMOS FONTE QDO i=1 E i=4
